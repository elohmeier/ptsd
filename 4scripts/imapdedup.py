...
def get_arguments(
    ...
    parser.add_argument(
        "-K", "--keyring", dest="keyring", help="Keyring name to get password"
    )
    parser.add_argument(
        "-w",
        "--password",
    ...
    try:
        if options_use_checksum:
            ...
            if options_use_id_in_checksum:
                update(
                    ("Message-ID:" + str_header(parsed_message, "Message-ID")).encode()
                )
...
def get_mailbox_list(
    server: imaplib.IMAP4, directory: str = '""', pattern: str = '"*"'
) -> List[str]:
    """
...
def get_matching_msgnums(
    server: imaplib.IMAP4, query: str, sent_before: Optional[str]
) -> List[int]:
    """
...
def get_undeleted_msgnums(
    server: imaplib.IMAP4, sent_before: Optional[str]
) -> List[int]:
    """
...
def mark_messages_deleted(
    server: imaplib.IMAP4, msgs_to_delete: List[int], only_tag: bool
):
...
def get_msg_headers(
    server: imaplib.IMAP4, msg_ids: List[int]
...
def process(options, mboxes: List[str]):
    ...
    try:
        ...
        for mbox in mboxes:
            ...
            # Select the mailbox
            msgs = check_response(
                server.select(mailbox=mbox, readonly=options.dry_run)
            )[0]
            ...
            if len(msgs_to_delete) == 0:
                ...
                if options.dry_run:
                    ...
                    if options.only_tag:
                        print(
                            "Tagging %i messages as '%s'..."
                            % (len(msgs_to_delete), TAG_NAME)
                        )
                    else:
                        print(
                            "Marking %i messages as deleted..." % (len(msgs_to_delete))
                        )
                    ...
                    for i in range(0, len(msgs_to_delete), chunkSize):
                        mark_messages_deleted(
                            server, msgs_to_delete[i : i + chunkSize], options.only_tag
                        )
                        if options.verbose:
