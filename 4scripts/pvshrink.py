...
while True:
    ...
    try:
        pvs = subprocess.check_output(
            [
                "pvs",
                "--segments",
                "-o",
                "lv_name,pv_segstart,pv_segsize",
                "--separator",
                ",",
                "--noheadings",
                device,
    ...
    verbose(
        "Remaining segments to move %d / %d "
        % (segments_to_move, total_segments_to_move)
    )
    ...
    print(
        "%d of %d (%.2f%%) done"
        % (
            total_segments_moved,
            total_segments_to_move,
            100 * float(total_segments_moved) / float(total_segments_to_move),
        )
    )
...
print(
    "Total size %d b + %d x %d b = %d b (%s)"
    % (metadata_size, total_allocated, pe_size, total_size, human_readable(total_size))
)

if not args.test:
    ret = subprocess.call(
        ["pvresize", "-v", "--setphysicalvolumesize", "%db" % total_size, device]
    )
    if ret != 0:
