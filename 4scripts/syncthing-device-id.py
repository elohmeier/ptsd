...
#!nix-shell -p openssl -p python3 -i python3
...
def luhn_checksum(data, alphabet=B32ALPHABET):
    ...
    result = (sum(number[::2]) + sum(sum(divmod(i * 2, n)) for i in number[1::2])) % n
    return alphabet[-result]


def main(incert):
    der_data = subprocess.check_output(
        ["openssl", "x509", "-outform", "DER"], stdin=incert
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate syncthing ID from certificate"
    )
