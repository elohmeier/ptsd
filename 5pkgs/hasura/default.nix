...
let
  pkgs = haskell.packages.ghc865.override {
    overrides = self: super: {
      # cabal2nix  --subpath server --maintainer offline --no-check --revision v1.1.1 https://github.com/hasura/graphql-engine.git
      hasura-graphql-engine = justStaticExecutables
        (
          (self.callPackage ./graphql-engine.nix {}).overrideDerivation (
            d: rec {
              name = "graphql-engine-${version}";

              # version in cabal file is not updated
              version = "1.1.1";
      ...
      # internal dependencies (find revisions in cabal.project file)
      ...
      # version constrained dependencies (find versions in graphql-engine.cabal)
      ...
      # unmark brokwn packages and do required modifications
