...
def get_specialisations(profile: Optional[str], generation: int) -> List[Tuple[Optional[str], int, Optional[str]]]:
    ...
    if not os.path.exists(specialisations_dir):
        return []
    return [(profile, generation, specialisation) for specialisation in os.listdir(specialisations_dir)]
...
def remove_old_entries(gens: List[Tuple[Optional[str], int]]) -> None:
    ...
    known_paths = []
    for gen in gens:
        known_paths.append(copy_from_profile(*gen, None, "kernel", True))
        known_paths.append(copy_from_profile(*gen, None, "initrd", True))
...
def main() -> None:
    ...
    for gen in gens:
        try:
            write_entry(*gen, None, machine_id)
            for specialisation in get_specialisations(*gen):
                write_entry(*specialisation, machine_id)
            if os.readlink(system_dir(*gen, None)) == args.default_config:
                write_loader_conf(*gen, None)
        except OSError as e:
