# Based on the stock module from 
# https://github.com/NixOS/nixpkgs/blob/master/nixos/modules/services/misc/gitea.nix
# Date: 2019-09-08, ref: 1e23007dcda878503aa26813aca783a59e6f04b3
# Added:
...

{
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        ...
        message = "ptsd.gitea.database.user must match ptsd.gitea.user if the database is to be automatically provisioned";
      }
    ];
    ...
    systemd.tmpfiles.rules = [
      ...
      "d '${cfg.stateDir}/custom/public' - ${cfg.user} gitea - -"
      "d '${cfg.stateDir}/custom/public/img' - ${cfg.user} gitea - -"
      "d '${cfg.stateDir}/custom/templates' - ${cfg.user} gitea - -"
      "d '${cfg.stateDir}/custom/templates/base' - ${cfg.user} gitea - -"
    ...
    systemd.services.gitea = {
      ...
      in
        ''
          ...
          cp -f ${../src/gitea/public/img/favicon.png} ${publicFolder}/img/favicon.png
          cp -f ${../src/gitea/public/img/gitea-lg.png} ${publicFolder}/img/gitea-lg.png
          cp -f ${../src/gitea/public/img/gitea-sm.png} ${publicFolder}/img/gitea-sm.png

          # copy templates
          cp -f ${../src/gitea/templates/base/footer_content.tmpl} ${templatesFolder}/base/footer_content.tmpl
          cp -f ${../src/gitea/templates/home.tmpl} ${templatesFolder}/home.tmpl
    ...
    ptsd.gitea.database.passwordFile =
      (
        mkDefault (
          toString (
            pkgs.writeTextFile {
