...

{
  services.openssh.hostKeys = [
    {
      type = "ed25519";
      path = "/nix/secrets/ssh.id_ed25519";
    }
  ];

  system.activationScripts.initialize-persistent =
    lib.stringAfter
      [
        "users"
        "groups"
  ...
  fileSystems =
    {
      ...
      "/etc/secureboot" = {
        device = "/nix/persistent/etc/secureboot";
        options = [ "bind" ];
      };
      "/var/lib/systemd" = {
        device = "/nix/persistent/var/lib/systemd";
        options = [ "bind" ];
      };
    }
    ...
    // lib.optionalAttrs config.services.tailscale.enable {
      "/var/lib/tailscale" = {
        device = "/nix/persistent/var/lib/tailscale";
        options = [ "bind" ];
      };
    }
    // lib.optionalAttrs config.ptsd.tailscale.cert.enable {
      "/var/lib/tailscale-cert" = {
        device = "/nix/persistent/var/lib/tailscale-cert";
        options = [ "bind" ];
      };
    }
    // lib.optionalAttrs (config.services.borgbackup.jobs != { }) {
      "/var/cache/borg" = {
        device = "/nix/persistent/var/cache/borg";
        options = [ "bind" ];
      };
    }
    // lib.optionalAttrs config.powerManagement.powertop.enable {
      "/var/cache/powertop" = {
        device = "/nix/persistent/var/cache/powertop";
        options = [ "bind" ];
      };
    };

  systemd.mounts =
    (lib.optional config.services.samba.enable {
