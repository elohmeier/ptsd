...
let
  ...
  mkManualBackupScript =
    name: cfg:
    pkgs.writeShellScriptBin "borgbackup-create-${name}" ''
      ...
      ${concatStringsSep "\n" (
        mapAttrsToList (name: value: "export ${name}=${escapeShellArg value}") (
          (mkPassEnv cfg) // cfg.environment
        )
      )}
      ${mkBackupScript cfg}
...
in
{
  ...
  config = mkIf (config.ptsd.borgbackup.jobs != { }) (
